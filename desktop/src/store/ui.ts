...
export const useUi = create<UiState>()(
  persist(
    (set) => ({
      ...
      // Never let all three detail zones close at once.
      toggleChat: () =>
        set((s) => (!s.showChat || s.showDiff || s.showTree ? { showChat: !s.showChat } : s)),
      toggleDiff: () =>
        set((s) => (!s.showDiff || s.showChat || s.showTree ? { showDiff: !s.showDiff } : s)),
      toggleTree: () =>
        set((s) => (!s.showTree || s.showChat || s.showDiff ? { showTree: !s.showTree } : s)),
