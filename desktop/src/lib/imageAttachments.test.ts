...
describe("image attachments", () => {
  it("accepts PNG and JPEG and rejects unsupported MIME/extensions", () => {
    ...
    expect(validateImageFiles([new File(["x"], "a.gif", { type: "image/gif" })])).toMatch(/PNG or JPEG/);
    expect(validateImageFiles([new File(["x"], "a.png", { type: "text/plain" })])).toMatch(/PNG or JPEG/);
  });
