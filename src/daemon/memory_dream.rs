...
pub fn dream_prompt(rows: &[(String, String, i64)]) -> String {
    let mut p = String::from("Verify facts from saved memory against the codebase — do they match code or not? If a memory claims something about ports, config, services, etc., check if it matches actual code/files. If mismatch or stale, propose superseded_by/merge/delete. If something requires user approval or is uncertain, write separately waiting for user answer on that question. Return JSON array of {proposal_type in [duplicate,contradiction,stale,merge,superseded_by,delete], target_ids: comma-separated ids, reason}. Decay heuristic: older last_accessed = staler. Memories:\n");
