//! Turning what Codex reports into quota windows.
//!
//! Two shapes say the same thing from different places: the `wham/usage` API
//! body, and the `rate_limits` line Codex writes into its rollout files. Both
//! are pure functions over JSON and files, kept apart from the fetching in
//! `limits::codex` so the network side stays small enough to read.
...
"#;
    ...
    fn wham_maps_real_fixture() {
        ...
        let a = map_wham(&body, 0, "codex:live", "live", true);
