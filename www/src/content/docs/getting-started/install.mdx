...
- **[GitHub CLI](https://cli.github.com/) (`gh`), authenticated.** Needed for **Open pull request** (`gh pr create` in `src/daemon/diff.rs`) and fetching attachments on private issues (`src/daemon/attachment.rs`). Backlog GitHub sync **prefers** a PAT (`repo` + `read:project` in Settings → Trackers) and falls back to `gh` (deprecated for backlog only — slow per-request process, will be removed for backlog; `gh` itself remains required for opening PRs, there is no PR list view in the app).
