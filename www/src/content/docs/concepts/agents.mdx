...
<Aside type="caution" title="Newer than the rest">
  Account switching is one of the more recent additions and is less settled than the surrounding app. Renaming and removing an account are there (removing deletes its vault); rough edges are worth [reporting](https://github.com/ephor/warpforge/issues).
</Aside>
