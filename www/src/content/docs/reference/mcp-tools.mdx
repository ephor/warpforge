...
## Tasks — turn discovered work into a queued task

### `create_task`

Creates a task on the board **without** auto-running an agent. It lands as *Queued* for a human to start.

```
create_task(prompt, project?, agent?, workflow?)
```
...
**Why it doesn't auto-run:** discovered work is exactly the work a human should triage. Queued costs nothing; a runaway agent chain costs a lot.
