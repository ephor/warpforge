...
Project runtime lives in `.warpforge/workspace.yaml`. The legacy `.warpforge.yaml`, `.wf.yaml`, and `.workspace.yaml` names (at the project root) are also supported — Warpforge checks them in that order and uses the first one it finds.
...
A list of Kubernetes port-forwards to keep alive alongside local services. Each entry:
