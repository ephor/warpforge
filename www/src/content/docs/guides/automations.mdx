...
2. **Who** — which agent runs it, and on which model. The model is per-automation: leave it on *agent default* to inherit whatever that harness last used, or pin one so an unattended run cannot silently get more expensive. Reasoning effort and mode, where the harness exposes them, are pinned the same way.
...
**Save & run once** saves the automation and immediately starts one run without touching the schedule — the fastest way to find out whether the prompt does what you meant before leaving it unattended for a week.
...
The panel under the picker describes the schedule in words and previews the **next three occurrences** in the automation's own timezone. If a schedule has no occurrence in the next year, it says so rather than sitting there looking scheduled.
...
<Aside type="tip">
  Worth saying out loud in your prompt what the automation is allowed to do. A scheduled run is an unattended run: it commits nothing on its own, but it does everything else a task can, and nobody is watching the turn.
</Aside>
